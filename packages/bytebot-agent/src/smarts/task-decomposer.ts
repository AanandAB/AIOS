...
export class TaskDecomposer {
   ...
   */
  async decompose(taskDescription: string): Promise<Subtask[]> {
  ...
  private identifySubtasks(taskDescription: string): Subtask[] {
    ...
    if (lowerDescription.includes('search') || lowerDescription.includes('find') || lowerDescription.includes('look')) {
      subtasks.push({
    ...
    if (lowerDescription.includes('click') || lowerDescription.includes('press') || lowerDescription.includes('button')) {
      subtasks.push({
    ...
    if (lowerDescription.includes('type') || lowerDescription.includes('enter') || lowerDescription.includes('write')) {
      subtasks.push({
        id: `action-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        type: 'action',
    ...
    if (lowerDescription.includes('read') || lowerDescription.includes('extract') || lowerDescription.includes('process')) {
      subtasks.push({
        id: `vision-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        type: 'vision',
    ...
    if (lowerDescription.includes('plan') || lowerDescription.includes('organize') || lowerDescription.includes('structure')) {
      subtasks.push({
