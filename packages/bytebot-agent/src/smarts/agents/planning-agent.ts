...
export class PlanningAgent {
  ...
  async createPlan(task: any, taskId: string): Promise<any> {
    this.logger.log(`Creating plan for task ${task.id} in task ${taskId}`);
    ...
    return {
      ...
      subtasks: [
        { id: 'subtask-1', type: 'vision', description: 'Identify UI elements' },
        { id: 'subtask-2', type: 'action', description: 'Click search button' },
        { id: 'subtask-3', type: 'action', description: 'Type search query' },
        { id: 'subtask-4', type: 'vision', description: 'Extract search results' }
      ],
      executionTime: 500,
      confidence: 0.92
    };
