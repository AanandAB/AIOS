...
export class SMARTSDemoService {
  ...
  async demonstrateSMARTS(): Promise<any> {
    ...
    this.logger.log('Initial system configuration:', JSON.stringify(initialConfig, null, 2));
    
    // 2. Process a complex task using recursive micro-agents
    const complexTask = "Research artificial intelligence trends in 2025 and create a summary report";
    ...
    const optimalConfig = await this.dynamicReconfigurator.analyzeTask(complexTask);
    this.logger.log('Optimal configuration for task:', JSON.stringify(optimalConfig, null, 2));
    ...
    const swapResult = await this.orchestrator.hotSwapComponent(componentId, newVersion);
    ...
    const newVisionAgentId = this.agentFactory.createAgent('vision', { version: '1.1.0' });
    const newActionAgentId = this.agentFactory.createAgent('action', { version: '1.1.0' });
    
    this.logger.log(`Created new agents: ${newVisionAgentId}, ${newActionAgentId}`);
    
    // 7. Process another task with the expanded agent pool
    const simpleTask = "Click the search button and type 'artificial intelligence'";
    const simpleTaskId = `simple-task-${Date.now()}`;
    
    this.logger.log(`Processing simple task: ${simpleTask}`);
    const simpleTaskResult = await this.orchestrator.processTask(simpleTask, simpleTaskId);
    
    // 8. Show final system state
    const finalConfig = this.dynamicReconfigurator.getCurrentConfiguration();
    this.logger.log('Final system configuration:', JSON.stringify(finalConfig, null, 2));
